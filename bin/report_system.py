...
################### Configuration ######################
version = "0.2"
if_list = ['eth0']	#需要监控的网卡
hd_list = ['/']		#需要监控的硬盘位置，对应df里的位置
reportcmd = "/ecim/agent/monitor/sysmon_set ConnService.Online"
interval = 60
########################################################
...
class ServerMonitor:
	...
	def get_cpu_load(self):
		cmd_get_cpu_load = "cat /proc/loadavg|awk '{print $1,$2,$3}'"
	...
	def get_cpu_use(self):
		cmd_get_cpu_num = "cat /proc/stat|grep 'cpu'|awk '{print $1}'"
		try:
		...
		cmd_get_cpu_use = 'cat /proc/stat| grep "cpu"'
		tmp1 = os.popen(cmd_get_cpu_use).read()
		sleep(1)
		tmp2 = os.popen(cmd_get_cpu_use).read()
		cpu_use = {}
		for cpu in CPUS:
			pattern = str(cpu) + r".*\n"
			m1 = re.search(pattern,tmp1)
			if m1 is not None:
				list1 = re.split('\s+',m1.group())
				try:
					idle_1 = float(list1[4])
					total_1 = float(list1[1]) + float(list1[2]) + float(list1[3]) + float(list1[4]) + float(list1[5]) + float(list1[6]) + float(list1[7])
				except:
					ErrorInfo = cpu + r":get_cpu_info_error"
					print ErrorInfo
					continue
			m2 = re.search(pattern,tmp2)
			if m2 is not None:
				list2 = re.split('\s+',m2.group())
				try:
					idle_2 = float(list2[4])
					total_2 = float(list2[1]) + float(list2[2]) + float(list2[3]) + float(list2[4]) + float(list2[5]) + float(list2[6]) + float(list2[7])
				except:
					ErrorInfo = cpu + r":get_cpu_info_error"
					print ErrorInfo
					continue
			cpu_use[cpu] = int((1 - (idle_2 - idle_1)/(total_2 - total_1))*100)
		return cpu_use

	
	#get udp connection information
	def get_udp(self):
		cmd_get_udp_info = "cat /proc/net/snmp|grep 'Udp:'|tail -n1|awk -F':' '{print $2}'"
		try:
	...
	def get_tcp(self):
		cmd_get_tcp_info = "cat /proc/net/snmp|grep 'Tcp:'|tail -n1|awk -F':' '{print $2}'"
		try:
...
if __name__ == "__main__":
	...
	# get cpu use, this method costs 1 second
	cpu_use = mon.get_cpu_use()
	#sleep interval-1 seconds
	sleep(interval-1)
	# report cpu_use
	if cpu_use != r'get_cpu_info_error':
		for key in cpu_use.keys():
			mon.report(key.upper() + "_Used(%)", cpu_use[key])
	# get mem use
	...
	if hd_io_1 !=  r'get_hd_io_error' and hd_io_2 != r'get_hd_io_error':
		...
		await = 0
		if io_read_use + io_write_use > 0 : await = (io_read_use + io_write_use) * 1000 / (io_read_req + io_write_req)
		io_use = long(hd_io_2['use']) - long(hd_io_1['use'])
		if io_use < 0 : io_use = io_use + 4294967296
		svctm = 0
		if io_read_use + io_write_use > 0 : svctm = (io_use * 1000)/ (io_read_req + io_write_req)
		# report to status

...
public:
    ...
    virtual void on_timer(uint8_t timer_id, tcp_client_channel* channel)
    {
		...
		if (timer_id == custom_min_id)
		{
			...
			if (!Name::BalanceMode::Client::pick(cluster_name, _sys_section, 1, v))
            {
				printf("Name::Client::pick error\n");
			}  
			else
			{
				if(!v.size())	
				{
					printf("Name::Client::pick empty\n");
				}
				else
				{
					tcp_client_channel* low_channel = Net::find_tcp_client_channel(g_loop, v[0]);
					if(!low_channel)
					{
						printf("find_tcp_client_channel fail: %s\n", string_by_addr(&v[0]).c_str());							
					}
					else
					{
						uint8_t	obuf[4096];
...
int main(int argc, char** argv)
{
	...
	for (uint16_t i = 0; i < num; i++)
	{
		...
		Name::BalanceMode::Client::add_watch(cluster_name);		

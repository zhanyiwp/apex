...
void aes_encrypt( aes_context *ctx, uint8_t input[16], uint8_t output[16] )
{
    ...
    GET_uint32_t( X0, input,  0 ); X0 ^= RK[0];
    GET_uint32_t( X1, input,  4 ); X1 ^= RK[1];
    GET_uint32_t( X2, input,  8 ); X2 ^= RK[2];
    GET_uint32_t( X3, input, 12 ); X3 ^= RK[3];
...
{                                               \
    ...
    PUT_uint32_t( X0, output,  0 );
    PUT_uint32_t( X1, output,  4 );
    PUT_uint32_t( X2, output,  8 );
    PUT_uint32_t( X3, output, 12 );
}
...
{
	aes_context ctx;
	uint32_t encrypt_step = 0;
	int32_t* suffix;
	uint8_t tmp[16];
	if(0==input_len || output_len < input_len+16)
		return -1;
	/*init ctx*/
	if(0!=aes_set_key(&ctx, key, nbits))
		return -1;
	/*encrypt*/
	while(encrypt_step < input_len)
	{
		if(encrypt_step+15>input_len)
		{
			memcpy(tmp, input+encrypt_step, input_len - encrypt_step);
			aes_encrypt(&ctx, tmp, output+encrypt_step);
			/*suffix block*/
			suffix = (int32_t*)(output + encrypt_step + 16);
			suffix[0] = random();
			suffix[1] = random();
			suffix[2] = random();
			suffix[3] = random();
			*(output + encrypt_step + 31) = (uint8_t)(input_len - encrypt_step);/*real bytes length in last block*/

			return encrypt_step+32;
		}
		else
		{
			aes_encrypt(&ctx, input+encrypt_step, output+encrypt_step);
			encrypt_step+=16;
		}
	}
	/*suffix block*/
	suffix = (int32_t*)(output + input_len);
	suffix[0] = random();
	suffix[1] = random();
	suffix[2] = random();
	suffix[3] = random();
	*(output + input_len + 15) = '\0';/*means real bytes length in last block is 16*/
	return input_len + 16;
}
int aes_decrypt_ecb(uint8_t *key, int nbits, uint8_t* input, uint32_t input_len, uint8_t* output, uint32_t output_len)
{
	aes_context ctx;
	uint32_t real_len, decrypt_step = 0;
	uint8_t tmp[16];
	if(input_len < 32 || input_len%16!=0 || output_len<input_len-32)
		return -1;
	/*init ctx*/
	if(0!=aes_set_key(&ctx, key, nbits))
		return -1;
	/*read suffix*/
	real_len = *(input + input_len - 1);
	if(real_len>15)
		return -1;
	input_len -= 16;
	/*decrypt*/
	while(decrypt_step < input_len)
	{
		if(0!=real_len && decrypt_step+16>=input_len)
		{
			aes_decrypt(&ctx, input+decrypt_step, tmp);
			memcpy(output+decrypt_step, tmp, real_len);
			return decrypt_step+real_len;
		}
		else
		{
			aes_decrypt(&ctx, input+decrypt_step, output+decrypt_step);
			decrypt_step+=16;
		}
	}
	return decrypt_step;
...
int aes_encrypt_cbc(uint8_t *key, int nbits, uint8_t* input, uint32_t input_len, uint8_t* output, uint32_t output_len)
{
	...
	while(encrypt_step < input_len)
	{
		if(encrypt_step+15>input_len)
		{
			...
			memcpy(u8, input+encrypt_step, input_len-encrypt_step);
			if(encrypt_step>0)
			{
#ifdef AES_USING_B128_OPTIMIZATION
				low = *(uint64_t*)(output+encrypt_step-16) ^ low;
				high = *(uint64_t*)(output+encrypt_step-8) ^ high;
#else
				int i=0;
				for(; i<16; ++i)
					u8[i] = output[encrypt_step-16+i] ^ u8[i];
#endif
			}
			aes_encrypt(&ctx, u8, output+encrypt_step);
			/*suffix block*/
			suffix = (int32_t*)(output + encrypt_step + 16);
			suffix[0] = random();
			suffix[1] = random();
		...
		else
		{
			...
			else
			{
				//xor
#ifdef AES_USING_B128_OPTIMIZATION
				low = *(uint64_t*)(output+encrypt_step-16) ^ *(uint64_t*)(input+encrypt_step);
				high = *(uint64_t*)(output+encrypt_step-8) ^ *(uint64_t*)(input+encrypt_step+8);
#else
				int i=0;
				for(; i<16; ++i)
					u8[i] = output[encrypt_step-16+i] ^ input[encrypt_step+i];
#endif
				aes_encrypt(&ctx, u8, output+encrypt_step);
...
int aes_decrypt_cbc(uint8_t *key, int nbits, uint8_t* input, uint32_t input_len, uint8_t* output, uint32_t output_len)
{
	...
	while(decrypt_step < input_len)
	{
		if(0!=real_len && decrypt_step+16>=input_len)
		{
			aes_decrypt(&ctx, input+decrypt_step, u8);
			if(decrypt_step>0)
			{
#ifdef AES_USING_B128_OPTIMIZATION
				low = *(uint64_t*)(input+decrypt_step-16) ^ low;
				high = *(uint64_t*)(input+decrypt_step-8) ^ high;
#else
				int i=0;
				for(; i<16; ++i)
					u8[i] = input[decrypt_step-16+i] ^ u8[i];
#endif
			}
			memcpy(output+decrypt_step, u8, real_len);
			return decrypt_step+real_len;
		}
		else
		{
			...
			{
#ifdef AES_USING_B128_OPTIMIZATION
				*(uint64_t*)(output+decrypt_step) = *(uint64_t*)(input+decrypt_step-16) ^ *(uint64_t*)(output+decrypt_step);
				*(uint64_t*)(output+decrypt_step+8) = *(uint64_t*)(input+decrypt_step-8) ^ *(uint64_t*)(output+decrypt_step+8);
#else
				int i=0;
				for(; i<16; ++i)
					output[decrypt_step+i] = input[decrypt_step-16+i] ^ output[decrypt_step+i];
#endif
			}
			decrypt_step+=16;
		}
